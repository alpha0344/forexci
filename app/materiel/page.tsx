...
export default function MaterielPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.validityTime || !formData.timeBeforeControl || !editingMaterial) {
    ...
    try {
      const body = {
        ...
        timeBeforeReload: formData.timeBeforeReload ? parseInt(formData.timeBeforeReload) : null
      }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-xl text-secondary max-w-2xl">
            Gérez vos modèles d'équipements de sécurité. Ces templates seront assignés à vos clients.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <h3 className="text-2xl font-bold text-primary mb-2">Aucun type de matériel trouvé</h3>
            <p className="text-secondary">
              Aucun template de matériel n'est actuellement configuré dans le système.
            </p>
          </div>
        ) : (
              ...
              <div key={material.id} className="card group">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-primary">{formatChangePeriod(material.validityTime)}</span>
                  </div>

                  <div className="flex justify-between">
                    <span className="text-secondary">Contrôle :</span>
                    <span className="font-semibold text-primary">{formatControlPeriod(material.timeBeforeControl)}</span>
                  </div>

                  {material.timeBeforeReload && (
                    <div className="flex justify-between">
                      <span className="text-secondary">Recharge :</span>
                      <span className="font-semibold text-primary">{formatChangePeriod(material.timeBeforeReload)}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Temps avant contrôle (jours) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    value={formData.timeBeforeControl}
                    onChange={(e) => setFormData({ ...formData, timeBeforeControl: e.target.value })}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Temps de validité (jours) <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    value={formData.validityTime}
                    onChange={(e) => setFormData({ ...formData, validityTime: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-orange"
                    placeholder="Ex: 1825"
                    min="1"
                    required
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData({ ...formData, timeBeforeReload: e.target.value })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-orange"
                    placeholder="Ex: 90"
                    min="1"
                  />
