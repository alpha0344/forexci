...
export default function UpdateVerificationModal({
  ...
  const handleUpdateVerifications = async () => {
    ...
    try {
      ...
      if (failedUpdates.length > 0) {
        throw new Error(
          `Échec de la mise à jour de ${failedUpdates.length} équipement(s)`,
        );
      }
  ...
  const handleFinalizePARecharges = async () => {
    ...
    try {
      ...
      if (failedUpdates.length > 0) {
        throw new Error(
          `Échec de la mise à jour de ${failedUpdates.length} recharge(s) PA`,
        );
      }
      ...
      setError(
        error instanceof Error
          ? error.message
          : "Erreur lors de la mise à jour des recharges",
      );
    } finally {
      setIsProcessing(false);
    }
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 mb-4">
                      Cette action va mettre à jour la{" "}
                      <strong>date de dernière vérification</strong> de tous les
                      équipements de ce client ({equipments.length} équipement
              ...
              <div className="space-y-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700 mb-4">
                      Toutes les dates de vérification ont été mises à jour avec
                      succès.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <ul className="text-sm text-blue-800 space-y-1 mb-4">
                    {paEquipments.map((equipment) => (
                      <li key={equipment.id}>• PA #{equipment.number}</li>
                    ))}
                  </ul>
                  <p className="text-sm text-blue-800 font-medium">
                    Des recharges PA ont-elles été effectuées lors de cette
                    vérification ?
                  </p>
                </div>
            ...
            {currentStep === "pa-recharge" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-700 mb-4">
                    Sélectionnez les équipements PA qui ont été rechargés et
                    indiquez la date.
                  </p>
                </div>
            ...
            {currentStep === "completed" && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 mb-6">
                  🎯 Toutes les vérifications ont été mises à jour avec la date
                  d'aujourd'hui.
                  {paRecharges.length > 0 &&
                    ` ${paRecharges.length} recharge(s) PA ont également été enregistrées.`}
                </p>
